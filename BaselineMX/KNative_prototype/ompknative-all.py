...
for service in services:
    output = subprocess.check_output(
         "docker run -d --name " + service + " --privileged --cpu-shares=0 yzzhangllm/" + service + "mx", shell=True).decode("utf-8")
...
output_file = open("omprun-all-out.txt", "w")
...
for load in loads:
    ...
    # inter_arrivals = []
    inter_arrivals = [interval] * int(rate * duration)
    # generate Poisson's distribution of events
    #inter_arrivals = []
    #np.random.seed(seed)
    #beta = 1.0 / rate
    ...
    #print(instance_events,flush=True)
    for service in services:
        ...
        for t in instance_events:
            ...
            threadToAdd = threading.Thread(target=lambda_func,
                                           args=("http://" + ip_addresses[services.index(service)] + ":9999",))
        ...
        print("=====================" + service + load_desc[loads.index(load)] + "=====================",
              file=output_file, flush=True)
        print(times,file=output_file, flush=True)
        print(len(times), file=output_file, flush=True)
